...
Emisor char(50),
Mensaje char(255),
FK_IDusuario char(6) not null foreign key (fk_IDusuario) references usuario(id_usuario)
)
GO
...
Fk_IDUsuario char(6) not null foreign key (Fk_IDUsuario) references usuario(id_usuario) -- a que clinica pertenece el doc
)
GO
...
Create table cita
(id_Cita int idenity not null primary key,
fecha date,
Descripcion varchar(255),
precio decimal,
...
fk_Usuario char(6) foreign key (fk_Usuario) references Usuario(ID_Usuario)
)
GO
...
INSERT INTO clinicas.usuario values('admin0','admin123', 'Admincl','admin','','',0,0,'m',null,1)
...
GO
	...
	--creando mantenimiento notificaciones
		...
		create procedure clinicas.verNotificacion(
			@id_usuario char(6)--//para verificar de quien es el mensaje
		)
		as
